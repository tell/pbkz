...
//Working option memo
// add to environment:  LD_LIBRARY_PATH=/usr/local/lib64/:
// compiling option: -lntl -lgmp -fopenmp -lgsl -std=c++11
...
void GenerateBKZStrategy(bkzstrategy& BS,mat_ZZ &L, stringmap& arg) {
    ...
    if (type=="shortvec") {
        ...
        if (arg["withenum"]=="true") {
			...
			int optime;
			conv (optime,cost / minM/1000);
			if (optime < 5) optime = 5; 
			logstream << " -optimize " << optime << " -preprocess " << optime << endl;
...
void GeneratePruningFunction(pruningfunction& ret,mat_ZZ &L, stringmap& arg) {
    ...
    if (vl>=1) {
        ...
        cout << "Cost=" << pruning_func::Rigid_upper_cost_pcvf(V,c,n,radius) << endl;
    }
    
    if (optimize>0) {
        RR cost = pruning_func::optimize_pruning_function(pf-1,radius,c,n,prob,vl,optimize,orderMI);
...
void LatticeENUM(mat_ZZ &L,pruningfunction& argpf, stringmap& arg) {
    ...
    if (pffilename!="") {
        ...
        set_approximate_extreme_pruning(pf,c,n,prob,0);
        pruning_func::invert(pf,0,n-1);
     }
    if (vl>=2) {
        cout << "Pruning function=[";
    ...
    if (arg["optimize"]!="") {
        ...
        pruning_func::init_pruning_func();
        RR cost = pruning_func::optimize_pruning_function(pf-1,radius,c-1,n,prob,vl,optimizesec,orderMI);
        if (vl>=3) {
            cout << "Pruning function=[";
            for (int i=0;i<n;i++) cout << pf[i] << " ";
            cout << "]" << endl;
    ...
    short** foundvec = lattice_enum::enum_share_foundvec;
    ...
    if (parallel>=2) {
        lattice_enum::ENUMCoremt(foundvec,foundlen,buff,findex,slide,0,n,elim,radius*radius,pf,mu,c,vl,parallel,enummode,finishmode);
    } else {
        lattice_enum::ENUMCore(foundvec,foundlen,buff,findex,slide,0,n,elim,radius*radius,pf,mu,c,vl,enummode);
    }
...
int main(int argc, char** argv) {
    ...
    for (i=1;i<argc;i++) {
        std::string a;
        a = argv[i];
        if ((a=="-genbasis") || (a=="-genmatrix") || (a=="-lll") || (a=="-bkz") || (a=="-genstrategy") || (a=="-enum") || (a=="-pfunc") || (a=="-disp") || (a=="-randomizebasis")) {
            a = "com=" + a.substr(1,a.length());
        }
        args.push_back(a);
    }
#endif    
